...
type ControllerInterface interface {
	...
	// This is specific to each job type because each job type can have its own logic to determine the succeeded, failed etc.
	// e.g. Check whether worker 0 or master is successfully exited to determine the job status
	UpdateJobStatus(
		job interface{},
